...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        # By the end of the while loop, L and R should converge to the minimum element's index
        return nums[L]
...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        while L <= R:
            M_speed = (L + R) // 2
            hour = calHour(M_speed)
            if hour <= h:
                res = min(res, M_speed)
                R = M_speed - 1  # Move left if condition is met
            else:
                L = M_speed + 1  # Move right otherwise
